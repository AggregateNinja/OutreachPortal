...
//        require_once('fpdi/fpdi.php');
        require_once('PDFMerger/tcpdf.php');
        require_once('PDFMerger/tcpdi.php');
    }
    ...
    public function merge($outputmode = 'browser', $outputpath = 'newfile.pdf') {
        ...
        $fpdi = new TCPDI(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        //$fpdi = new TCPDI(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'ISO-8859-1', false);

        $fpdi->setPDFVersion('1.7');
        ...
        foreach ($this->_files as $file) {
            ...
            if ($filepages == 'all') {
                for ($i = 1; $i <= $count; $i++) {
                    ...
                    //$fpdi->AddPage(PDF_PAGE_ORIENTATION, array($size['w'], $size['h']));

                    // https://github.com/rguedes/PDFMerger/blob/master/Classes/PDFMerger.php
                    $orientation = ($size['h'] > $size['w']) ? 'P' : 'L';

//                    error_log($size['w'] . " - " . $size['h'] . " - " . $orientation);
//                    if ($orientation == 'L') {
//                        $size['h'] = ($size['h'] / $size['w']) * 8.5;
//                        $size['w'] = 8.5;
//                    }
                    ...
                    // Calling this function allows PDF form fields to be included in the merge
                    // Make sure tcpdi.php and tcpdi_parser are updated to the newer versions.
                    // TCPDI:           https://github.com/pauln/tcpdi
                    // TCPDI_PARSER:    https://github.com/pauln/tcpdi_parser
                    // https://github.com/pauln/tcpdi/issues/8#issuecomment-223324997
                    $fpdi->importAnnotations($i);
                }
            } else {
