...
require_once 'jasper/src/Jaspersoft/Tool/MimeMapper.php';
require_once 'jasper/src/Jaspersoft/Service/Result/SearchResourcesResult.php';
...
class RepositoryService
{
    ...
    public function updateResource(Resource $resource, $overwrite = false)
    {
        ...
        return $resource::createFromJSON(
    ...
    public function createFileResource(File $resource, $binaryData, $parentFolder, $createFolders = true)
    {
        ...
        $url .= '?' . Util::query_suffix(array("createFolders" => $createFolders));
        $body = $binaryData;
        $response = $this->service->sendBinary(
            $url,
            array(201, 200),
            $body,
            MimeMapper::mapType($resource->type),
            'attachment; filename=' . $resource->label,
