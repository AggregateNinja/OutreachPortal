    ...
    */
    ...
    public function getSalesGroupMembers(array $input) {
        ...
        $sql = "
            ...
            LEFT JOIN " . self::DB_CSS . "." . self::TBL_SALESGROUP . " sg ON sgl.salesGroupId = sg.id
            $where
            ORDER BY groupName, e.lastName ASC, e.firstName ASC
        ";
          ...
          FROM (
              SELECT
                    ...
                    ROUND(CASE
                        WHEN cr2.idCommissions IS NOT NULL AND cr2.commissionRate > 0  AND SUM(dcc.paid) >= cr2.minPayment THEN 
                            CASE
                                WHEN s2.byPercentage = 1 THEN (SUM(dcc.paid) * (cr2.commissionRate/100)) - ((SUM(dcc.paid) * (cr2.commissionRate/100)) * (cr.commissionRate/100))
                                ELSE cr2.commissionRate
                            END
                        WHEN (cr2.idCommissions IS NULL OR cr2.commissionRate <= 0) AND s2.commisionRate > 0 THEN
                            CASE
                                WHEN s2.byPercentage = 1 THEN (SUM(dcc.paid) * (s2.commisionRate/100)) - ((SUM(dcc.paid) * (s2.commisionRate/100)) * (s.commisionRate/100))
                                ELSE s2.commisionRate
                            END
                    ...
                    ROUND(CASE
                        WHEN cr.idCommissions IS NOT NULL AND cr.commissionRate > 0  AND SUM(dcc.paid) >= cr.minPayment THEN 
                            CASE
                                WHEN s.byPercentage = 1 THEN (SUM(dcc.paid) * (cr2.commissionRate/100)) * (cr.commissionRate/100)
                                ELSE cr.commissionRate
                            END
                        WHEN (cr.idCommissions IS NULL OR cr.commissionRate <= 0) AND s.commisionRate > 0 THEN
                            CASE
                                WHEN s.byPercentage = 1 THEN (SUM(dcc.paid) * (s2.commisionRate/100)) * (s.commisionRate/100)
                                ELSE s.commisionRate
                            END
...
//            ORDER BY o.DOI ASC";
        ...
        $sql = "
            SELECT 	DISTINCT o.idOrders, o.accession, do.iddetailOrders, dcc.iddetailCptCodes,
                    ...
                    #cr.idCommissions, cr.minPayment, cr.commissionRate,
                    #s.byPercentage, s.commisionRate AS `defaultCommissionRate`,
